...
- frontier-F.acceptance — Docker is not installed in this local environment, so `make deploy`, Compose health, and live browser WebSocket tick delivery could not be verified here. **To unblock:** run `make deploy` on a host with Docker/Compose or install Docker Desktop locally, then verify `http://localhost:3000`, `http://localhost:8000/api/health`, the worker logs, and a connected `/ws/markets/{code}` stream.
